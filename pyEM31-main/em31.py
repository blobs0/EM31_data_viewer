...
def extract_gps(r31_dat, epoch_ms, epoch_ts):
    ...
    gps_df = pd.DataFrame(
        data={
            ...
            "fix": pd.to_numeric(pd.Series(gga_data[:, 2]), errors="coerce")
            .fillna(0)
            .astype("uint8"),
            "nsats": pd.to_numeric(pd.Series(gga_data[:, 3]), errors="coerce")
            .fillna(0)
            .astype("uint16"),
            "hdop": pd.to_numeric(pd.Series(gga_data[:, 4]), errors="coerce").astype(
                "float32"
            ),
            "alt": pd.to_numeric(pd.Series(gga_data[:, 5]), errors="coerce").astype(
                "float32"
            ),
            "lat": pd.to_numeric(pd.Series(gga_data[:, 6]), errors="coerce").astype(
                "float64"
            ),
            "lat_dir": pd.Series(gga_data[:, 7]).astype(pd.StringDtype()),
            "lon": pd.to_numeric(pd.Series(gga_data[:, 8]), errors="coerce").astype(
                "float64"
            ),
